      ...
      try {
        ...
        const secured =
          process.platform === 'win32'
            ...
            : secureVerificationCommand(executable, argumentsWithContext);
...
public static class AutoCodeQaJob {
  ...
  public static int Run(string command,string[] arguments,string cwd,int maxOutputBytes,int parentPid) {
    IntPtr job=IntPtr.Zero,output=IntPtr.Zero,error=IntPtr.Zero,input=IntPtr.Zero,parent=IntPtr.Zero;
    ProcessInfo process=new ProcessInfo();
    string outputPath=Path.GetTempFileName(),errorPath=Path.GetTempFileName();
    try {
      ...
      if(new FileInfo(outputPath).Length+new FileInfo(errorPath).Length>maxOutputBytes) return -1;
      ...
      File.Delete(outputPath); File.Delete(errorPath);
    }
  }
}`;
