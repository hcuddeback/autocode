...
Common Git metadata outside authorized roots requires explicit directory read authorization or a registered linked worktree whose Git metadata back-pointer resolves to the actual worktree's `.git` file. Repository-controlled pointers cannot authorize unrelated private repositories. Automatically discovered directories do not expand the operator authorization set.

Writable enclaves inside `.autocode` are limited to `.autocode/worktrees` infrastructure with a registered worktree, an external common directory and a regular bounded back-pointer. Plain Git markers cannot reclassify cache/run state or standalone repositories as writable infrastructure. Standard hidden `.credentials` and `.secrets` directories remain credential boundaries when directly authorized or nested in state, including opaque filenames.

Actual Git `config` and `config.worktree` locations are parsed with Git's bounded, null-delimited config reader without following includes. Configs with URL user information, authentication headers, credential settings, private key/cookie references, include directives or embedded authentication/filter helper commands, including `core.askPass`, receive credential isolation. Public refs and worktree identifiers named `config` remain data labels. Ordinary public configs, including valueless boolean settings, remain readable. Unsafe or invalid inspection fails closed without retaining raw Git errors that could include secrets. Credential-bearing configs can make Git commands requiring those files unavailable; no secret-bearing config or authentication fallback is exposed.
