...
Configured PR review then found that arbitrary string values inside opaque adapter evidence could bypass field-name-based receipt redaction. Runner evidence subtrees are now fully string-redacted while separately validated runner/model/execution/effect controls remain stable. A subsequent exact-head review found that adapter/runtime changes were absent from receipt identity and that independently bounded result fields could exceed the receipt limit when combined. Receipts now bind the adapter revision, capabilities, assignment, preflight configuration, and resolved executable/resource content identity; a changed executable invalidates completed evidence without replay. Runner results now have one aggregate 512-KiB serialized limit, safely below the receipt limit. Focused regressions and the complete 321-test run pass. Review-fix commit `48c39d0` is published; no review finding is deferred.

Verified review-fix commit `48c39d0` is published on `feat/AC-015-configurable-role-runners` in [PR #25](https://github.com/hcuddeback/autocode/pull/25). The configured findings are fixed with focused coverage and complete verification. The active task remains in review; exact-head re-review, conversation resolution, configured checks, human merge authorization, and post-merge workbook reconciliation remain pending. `tasks/README.md` is intentionally unchanged.
