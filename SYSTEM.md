...
- Run and phase identifiers are canonical lowercase path segments; each invocation verifies that its run path remains untracked and gitignored, rejects credential-bearing definitions, and redacts known environment/workspace credentials from untrusted adapter reasons before durable persistence.
