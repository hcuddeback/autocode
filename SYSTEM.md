...
| Local workflow is integrated   | AC-012 phase fixtures, including process interruption | Full suite after PR review corrections: 259 passed, four platform skips, no failures |
...
- Timeout and output-limit termination escalate to forceful process-tree shutdown and return within a fixed grace period. Windows Codex sessions use kill-on-close Job Objects; Linux and macOS execution fail closed pending accepted containment.
...
- Windows verification uses Job Objects for native commands and contained CMD/BAT shims with escaped arguments and delayed expansion disabled; Linux and macOS execution fail closed pending accepted containment.
